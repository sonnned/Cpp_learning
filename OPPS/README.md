...
Son tres tipos de relaciones entre clases. La composición es una relación fuerte, la agregación es una relación débil y la asociación es una relación más débil que la agregación
## Composición
Una clase es componente de otra clase. Dadas dos clases, la clase A y la clase B, se encuentran en una relación parte-de-tiene-una si la clase A forma parte de la clase B o viceversa.
Una instancia de la clase componente sólo puede crearse dentro de la clase principal
```cpp
#include <iostream>
using namespace std;
...
```
## Asociación
Se trata de una relación algo menos concreta entre dos clases. La clase A y la clase B tienen una relación de uso si una o ambas necesitan el objeto de la otra para realizar una operación, pero ambas clases pueden existir independientemente la una de la otra.
Esto implica que una clase tiene una referencia al objeto de la otra clase, pero no decide el tiempo de vida del objeto referenciado de la otra clase.
```cpp
...
```
## Agregación
La agregación es muy similar a la composición. También sigue el modelo Has-A. Esto crea una relación padre-hijo entre dos clases, con una clase que posee el objeto de otra.
```cpp
#include <iostream>
#include <string>
using namespace std;
